...
hit the network (~2-5s per unknown ticker for yfinance metadata) so the
...
def search_universe(
    ...
    Simple deterministic ranking — no fuzzy library needed at this scale
    (~503 SP500 + handful of user-added rows).
    """
...
class EnsureRequest(BaseModel):
    tickers: List[str]
    # 2y default matches the worker so the MFC factor windows have buffer.
    period: str = "2y"
...
def ensure_cached(req: EnsureRequest):
    ...
    Workflow:
      ...
      4. Persist the symbols so the worker refreshes them on the next tick.
    ...
    # 3. Persist for the worker. Only persist `added` — no point telling
